// IBKeymaster v2 CUDA Kernel
// On macOS: stub only (Metal is used instead)
// On Linux/Windows: core keyer + guided filter (TODO: implement full GPU path)
...
__global__ void IBKeymasterKernel(int p_Width, int p_Height,
                   ...
                   const float* p_Background, float* p_Output)
{
    ...
    // NOTE: Guided filter on CUDA not yet implemented — runs core keyer only.
    // The CPU fallback handles guided filter when CUDA is used.
    ...
    IBKeymasterKernel<<<blocks, threads, 0, stream>>>(
        p_Width, p_Height,
        ...
        p_BlackClip, p_WhiteClip,
        p_Input, p_Screen, p_Output);

    cudaError_t err = cudaGetLastError();
    if (err != cudaSuccess) {
        printf("IBKeymaster CUDA kernel launch failed: %s\n", cudaGetErrorString(err));
        return;
    }
    if (stream) {
